...
CString& CString::insert(size_t pos, const CString& str, size_t subpos, size_t sublen) // –аботатет но вызывает переполнение пам€ти после return 0
{
    this->reserve(sublen);
    int counter = 0;
    char* new_data;
...
CString& CString::insert(size_t pos, const char* s, size_t n) // –аботатет но вызывает переполнение пам€ти после return 0
{
    size_t new_size = 0;
    while (s[new_size] != '\0')
    {
...
CString& CString::insert(size_t pos, size_t n, char c) // –аботатет но вызывает переполнение пам€ти после return 0
{
    this->reserve(n);
    int counter = 0;
    char* new_data;
...
CString& CString::replace(size_t pos, size_t len, const char* s, size_t n) // Ќе работает, утечка пам€ти после return 0
{
    int sublen = 0;
    while (s[sublen] != '\0')
    {
