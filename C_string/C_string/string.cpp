...
CString& CString::insert(size_t pos, const char* s, size_t n) // –аботатет но вызывает утечку пам€ти после return 0
{
    size_t new_size = 0;
    while (s[new_size] != '\0')
    {
...
CString& CString::insert(size_t pos, size_t n, char c) // –аботатет но вызывает утечку пам€ти после return 0
{
    this->reserve(n);
    int counter = 0;
    char* new_data;
